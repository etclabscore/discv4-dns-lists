...
for network in "$@"; do
    ...
    devp2p nodeset filter all.json -limit 400 -eth-network "$network" >"all.${network}.${ETH_DNS_DISCV4_PARENT_DOMAIN}/nodes.json"

    mkdir -p "les.${network}.${ETH_DNS_DISCV4_PARENT_DOMAIN}"
    devp2p nodeset filter all.json -limit 400 -les-server -eth-network "$network" >"les.${network}.${ETH_DNS_DISCV4_PARENT_DOMAIN}/nodes.json"

    mkdir -p "snap.${network}.${ETH_DNS_DISCV4_PARENT_DOMAIN}"
    devp2p nodeset filter all.json -limit 400 -snap -eth-network "$network" >"snap.${network}.${ETH_DNS_DISCV4_PARENT_DOMAIN}/nodes.json"
